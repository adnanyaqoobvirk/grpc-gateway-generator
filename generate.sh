...
find ${PROTOS_PATH} -name "*.proto" -and -not -path '*/..*' -exec protoc -I${PROTOS_PATH} \
...
PACKAGES=$(find ${PROTOS_PATH} -name "*.proto" -and -not -path '*/..*'-print | xargs grep -E "package( )+[^=\"]*;" -ho | uniq | sed 's/;/ /g')
for PACKAGE in ${PACKAGES}
do
